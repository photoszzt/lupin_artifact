...
typedef struct qspinlock {
  atomic_t val;
} arch_spinlock_t __attribute__((aligned(CACHELINE_SIZE)));
...
void mcs_init_locks(uint64_t *lock, unsigned long cores, bool cxl) {
  (void)lock;
  size_t size = 4 * cores * sizeof(struct mcs_spinlock);
  if (cxl) {
    mcs_pool = (struct mcs_spinlock *)cxlalloc_memalign(size, CACHELINE_SIZE);
    assert(mcs_pool != NULL);
  } else {
    int ret = posix_memalign((void **)&mcs_pool, CACHELINE_SIZE, size);
    assert(ret == 0);
  }
  memset(mcs_pool, 0, size);
}
...
static __always_inline void clear_pending_set_locked(struct qspinlock *lock) {
  struct __qspinlock *l = (struct __qspinlock *)(void *)lock;
...
static __always_inline u32 xchg_tail(struct qspinlock *lock, u32 tail) {
   ...
   */
  return (u32)xchg_release16((uint16_t *)&l->tail, tail >> _Q_TAIL_OFFSET)
         << _Q_TAIL_OFFSET;
}
...
void queued_spin_lock_slowpath(struct qspinlock *lock, u32 val,
  ...
  for (;;) {
    ...
    if (val == new_val)
      new_val |= _Q_PENDING_VAL;

    /*
     * Acquire semantic is required here as the function may
     * return immediately if the lock was free.
     */
    old = atomic_cmpxchg_acquire32((uint32_t *)&lock->val, val, new_val);
    if (old == val)
      break;
...
unsigned long __attribute__((noinline)) lock_acquire(uint64_t *lock,
                                                     unsigned long threadnum) {
  u32 val;
