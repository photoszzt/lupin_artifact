...
#include "cxlalloc.h"
#include "include/lk_atomics.h"
#include "include/lk_barrier.h"
#include "include/lk_cmpxchg.h"
#include <errno.h>
#include <stdbool.h>
#include <stdio.h>
#include <string.h>
...
// #define SPIN_TAIL_ALIGNMENT 64UL
...
static inline void osq_lock_init(uint64_t *lock, unsigned long cores,
  ...
  if (cxl) {
    global_osq_nodepool_ptr = (struct optimistic_spin_node *)cxlalloc_memalign(
        size, SPIN_NODE_ALIGNMENT);
    global_osq = (struct optimistic_spin_queue *)cxlalloc_memalign(
        sizeof(struct optimistic_spin_queue), SPIN_TAIL_ALIGNMENT);
  } else {
    global_osq_nodepool_ptr =
        (struct optimistic_spin_node *)aligned_alloc(SPIN_NODE_ALIGNMENT, size);
    global_osq = (struct optimistic_spin_queue *)aligned_alloc(
        SPIN_TAIL_ALIGNMENT, sizeof(struct optimistic_spin_queue));
  }

  if (global_osq_nodepool_ptr == NULL)
    exit(errno);
   ...
   * spinning thread uses a different backoff sleep time, and we can adjust the
   * maximum sleep time by redefine MAX_BACKOFF_SLEEP_US or tuning via parameter
   * '-s' By default, we disable this sleep (MAX_BACKOFF_SLEEP_US = 0)
   *
...
osq_wait_next(struct optimistic_spin_queue *lock,
              ...
              struct optimistic_spin_node *prev, unsigned long cpu_number) {
...
unqueue:
  ...
  for (;;) {
    if (prev->next == node && cmpxchg(&prev->next, node, NULL) == node)
      break;
...
static void osq_unlock(uint64_t *osq, unsigned long cpu_number) {
   ...
   */
  if (atomic_cmpxchg_release(&lock->tail, curr, OSQ_UNLOCKED_VAL) == curr)
    return;
