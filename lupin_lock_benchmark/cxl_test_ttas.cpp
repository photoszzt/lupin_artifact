...
static void *measure_lpttas(void *arg) {
  ...
  printf("[%lu] TTAS_cxl mean passage latency (us): %lf\n", thread_id,
...
int main(void) {
  ...
  /* init cxlalloc */
  cxlalloc_init("JJ", default_cxl_mem_size, THREADS_NUM, THREADS_NUM + 1, 0, 1);
  printf("cxlalloc init succeed!, size = %ld bytes\n", default_cxl_mem_size);

  global_lock = (struct ttas_mutex *)cxlalloc_malloc(sizeof(struct ttas_mutex));
  global_lock->spin_lock = UNLOCKED;
  global_count =
      (std::atomic<uint64_t> *)cxlalloc_malloc(sizeof(std::atomic<uint64_t>));
