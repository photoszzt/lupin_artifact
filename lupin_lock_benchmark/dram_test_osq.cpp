...
static void *measure_osq(void *arg) {
  ...
  printf("[%lu] osq_dram mean passage latency (us): %lf\n", thread_id,
...
int main(void) {
  ...
  osq_lock_init(&global_lock, THREADS_NUM, false);
  ...
  for (int i = 0; i < THREADS_NUM; i++) {
    uint64_t vid = i;
    pthread_create(&p_thds[i], &attr, measure_osq, (void *)vid);
  }
  pthread_barrier_wait(&barrier);
