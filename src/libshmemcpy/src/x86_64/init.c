...
#define SHMEM_F_MEM_MOVNT (SHMEM_F_MEM_WC | SHMEM_F_MEM_NONTEMPORAL)
#define SHMEM_F_MEM_MOV   (SHMEM_F_MEM_WB | SHMEM_F_MEM_TEMPORAL)

static void *
shmem_memmove_nodrain(void *dest, const void *src, size_t len, unsigned flags,
		flush_func flushf, const struct memmove_nodrain *memmove_funcs)
{
	...
	else if (flags & SHMEM_F_MEM_MOVNT)
		memmove_funcs->nt.flush(dest, src, len);
	else if (flags & SHMEM_F_MEM_MOV)
...
static void *
shmem_memmove_nodrain_eadr(void *dest, const void *src, size_t len,
		...
		const struct memmove_nodrain *memmove_funcs)
{
	...
	if (flags & SHMEM_F_MEM_NOFLUSH)
		memmove_funcs->t.noflush(dest, src, len);
	else if (flags & SHMEM_F_MEM_NONTEMPORAL)
...
static void *
shmem_memset_nodrain(void *dest, int c, size_t len, unsigned flags,
...
static void *
shmem_memset_nodrain_eadr(void *dest, int c, size_t len, unsigned flags,
		flush_func flushf, const struct memset_nodrain *memset_funcs)
{
	/* suppress unused-parameter errors */
	SUPPRESS_UNUSED(flushf);
...
static void
shmem_set_mem_funcs(struct shmemcpy_arch_api *info)
{
	info->memmove_nodrain = shmem_memmove_nodrain;
	info->memmove_nodrain_eadr = shmem_memmove_nodrain_eadr;
	info->memset_nodrain = shmem_memset_nodrain;
	info->memset_nodrain_eadr = shmem_memset_nodrain_eadr;
}
 ...
 */
static void
use_sse2_memcpy_memset(struct shmemcpy_arch_api *info, enum memcpy_impl *impl,
		int wc_workaround)
{
#if SSE2_AVAILABLE
	*impl = MEMCPY_SSE2;

	shmem_set_mem_funcs(info);
 ...
 */
static void
use_avx_memcpy_memset(struct shmemcpy_arch_api *info, enum memcpy_impl *impl,
		int wc_workaround)
{
#if AVX_AVAILABLE
	LOG(3, "avx supported");

	char *e = getenv("SHMEM_AVX");
	if (e != NULL && strcmp(e, "0") == 0) {
		LOG(3, "SHMEM_AVX set to 0");
		return;
	}

	LOG(3, "SHMEM_AVX enabled");
	*impl = MEMCPY_AVX;
 ...
 */
static void
use_avx512f_memcpy_memset(struct shmemcpy_arch_api *info,
		enum memcpy_impl *impl)
{
#if AVX512F_AVAILABLE
	LOG(3, "avx512f supported");

	char *e = getenv("SHMEM_AVX512F");
	if (e != NULL && strcmp(e, "0") == 0) {
		LOG(3, "SHMEM_AVX512F set to 0");
		return;
	}

	LOG(3, "SHMEM_AVX512F enabled");
	*impl = MEMCPY_AVX512F;
 ...
 */
static void
use_movdir64b_memcpy_memset(struct shmemcpy_arch_api *info,
		enum memcpy_impl *impl)
{
#if MOVDIR64B_AVAILABLE
	LOG(3, "movdir64b supported");

	char *e = getenv("SHMEM_MOVDIR64B");
	if (e == NULL || strcmp(e, "0") == 0) {
		LOG(3, "SHMEM_MOVDIR64B set to 0");
		return;
	}

	LOG(3, "SHMEM_MOVDIR64B enabled");
...
/*
 * shmem_get_cpuinfo -- configure libshmem based on CPUID
 */
static void
shmem_cpuinfo_to_funcs(struct shmemcpy_arch_api *info, enum memcpy_impl *impl)
{
	LOG(3, NULL);
	...
	if (is_cpu_clflushopt_present()) {
		...
		char *e = getenv("SHMEM_NO_CLFLUSHOPT");
		if (e && strcmp(e, "1") == 0) {
			LOG(3, "SHMEM_NO_CLFLUSHOPT forced no clflushopt");
		} else {
	...
	if (is_cpu_clwb_present()) {
		...
		char *e = getenv("SHMEM_NO_CLWB");
		if (e && strcmp(e, "1") == 0) {
			LOG(3, "SHMEM_NO_CLWB forced no clwb");
		} else {
	...
	char *ptr = getenv("SHMEM_WC_WORKAROUND");
	if (ptr) {
		if (strcmp(ptr, "1") == 0) {
			...
			LOG(3, "incorrect value of SHMEM_WC_WORKAROUND (%s)",
				ptr);
		}
	}
	...
	ptr = getenv("SHMEM_NO_MOVNT");
	if (ptr && strcmp(ptr, "1") == 0) {
		LOG(3, "SHMEM_NO_MOVNT forced no movnt");
	} else {
		/*
		 * shmem_set_mem_funcs is not used at all when all available
		 * operations are disabled
		 */
		SUPPRESS_UNUSED(shmem_set_mem_funcs);
...
/*
 * shmemcpy_arch_api_init -- initialize architecture-specific list of shmem_api operations
 */
void
shmemcpy_arch_api_init(struct shmemcpy_arch_api *info)
{
	...
	shmem_cpuinfo_to_funcs(info, &impl);

#if !defined(__KERNEL__) && !defined(MODULE)
	/*
	 * For testing, allow overriding the default threshold
	 * for using non-temporal stores in shmem_memcpy_*(), shmem_memmove_*()
	 * and shmem_memset_*().
	 * It has no effect if movnt is not supported or disabled.
	 */
	const char *ptr = getenv("SHMEM_MOVNT_THRESHOLD");
	if (ptr) {
		long long val = atoll(ptr);

		if (val < 0) {
			LOG(3, "Invalid SHMEM_MOVNT_THRESHOLD");
		} else {
			LOG(3, "SHMEM_MOVNT_THRESHOLD set to %zu", (size_t)val);
