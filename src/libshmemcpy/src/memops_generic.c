 ...
 *
 * This fallback is needed to fulfill guarantee that shmem_mem[cpy|set|move]
 ...
 * This guarantee is needed to maintain correctness.
...
/*
 * shmem_flush_flags -- internal wrapper around shmem_flush
 */
static inline void
shmem_flush_flags(const void *addr, size_t len, unsigned flags,
		flush_func flush)
{
		...
		flush_func flush, const struct memmove_nodrain *memmove_funcs)
{
	...
	LOG(15, "pmemdest %p src %p len %zu flags 0x%x", dst, src, len, flags);
	...
	if ((uintptr_t)cdst - (uintptr_t)csrc >= len) {
		...
		if (cnt > 0) {
			...
			shmem_flush_flags(cdst, cnt, flags, flush);
		...
		dst8 = (uint64_t *) (const void*)cdst;
		src8 = (const uint64_t *) (const void*)csrc;

		while ((len >= 128) && (bool)(CACHELINE_SIZE == 128)) {
			cpy128(dst8, src8);
			shmem_flush_flags(dst8, 128, flags, flush);
			len -= 128;
			dst8 += 16;
			src8 += 16;
		}
		...
		while (len >= 64) {
			cpy64(dst8, src8);
			shmem_flush_flags(dst8, 64, flags, flush);
			len -= 64;
			dst8 += 8;
			src8 += 8;
		}
		...
		if (remaining)
			shmem_flush_flags(cdst - remaining, remaining, flags,
					flush);
	} else {
		cdst += len;
		csrc += len;
		...
		uint64_t *dst8 = (uint64_t *)(void *)cdst;
		const uint64_t *src8 = (const uint64_t *)(const void *)csrc;
		...
		if (remaining)
			shmem_flush_flags(cdst, remaining, flags, flush);
	}
		...
		flush_func flush, const struct memset_nodrain *memset_funcs)
{
	...
	if (remaining)
		shmem_flush_flags(cdst - remaining, remaining, flags, flush);
	return dst;
}
