...
static void
vout(int flags, const char *prepend, const char *fmt, va_list ap) __attribute__ ((format (printf, 3, 0)));

static void
out(int flags, const char *fmt, ...) __attribute__ ((format (printf, 2, 3)));
...
vout(int flags, const char *prepend, const char *fmt, va_list ap)
{
	...
	ret = fputs(buf, Tracefp);
	if (ret == EOF) {
		UT_FATAL("fputs failed");
	}
	if (flags & OF_ERR) {
		ret = fputs(buf, Errfp);
		if (ret == EOF) {
...
static void
out(int flags, const char *fmt, ...) FORMAT_PRINTF(2, 3);
...
{
	int ret = 0;
	if (Outfp != NULL && Outfp != stderr && Outfp != stdout) {
		ret = fclose(Outfp);
		if (ret == EOF) {
			perror("fclose");
			abort();
	...
	if (Errfp != NULL && Errfp != stderr && Errfp != stdout) {
		ret = fclose(Errfp);
		if (ret == EOF) {
			perror("fclose");
			abort();
...
check_open_files(void)
{
	...
	if (Fd_errcount) {
		if (getenv("UNITTEST_DO_NOT_FAIL_OPEN_FILES"))
			UT_OUT(
    ...
    const char *fmt, va_list ap)
{
	...
	char *ignore_bb =
		getenv("UNITTEST_CHECK_OPEN_FILES_IGNORE_BADBLOCKS");
	...
	if (getenv("UNITTEST_LOG_LEVEL") != NULL)
		LogLevel = atoi(getenv("UNITTEST_LOG_LEVEL"));
	else
		LogLevel = 2;
