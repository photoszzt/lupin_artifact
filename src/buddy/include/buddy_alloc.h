...
/* Find the buddy tree position */
struct buddy_tree_pos buddy_malloc_find_pos(struct buddy *buddy, size_t requested_size);
/* Apply the tree position for allocation */
void *buddy_malloc_apply(struct buddy *buddy, struct buddy_tree_pos);
...
/* Find the buddy tree position to allocate */
struct buddy_tree_pos buddy_malloc_find_pos(struct buddy *buddy, size_t requested_size);
/* Allocate the slot specified by the buddy tree */
void *buddy_malloc_apply(struct buddy *buddy, struct buddy_tree_pos);

/* Find the buddy tree position to free */
struct buddy_tree_pos buddy_free_find_pos(struct buddy *buddy, void *ptr, size_t requested_size);
/* Free the slot specified by the buddy tree */
void buddy_free_apply(struct buddy *buddy, struct buddy_tree_pos pos);
...
static struct buddy_tree *buddy_tree_init(unsigned char *at, uint8_t order, size_t *size);
...
static force_inline size_t buddy_tree_depth(struct buddy_tree_pos pos);

/* Returns the left child node position. Does not check if that is a valid position */
static force_inline struct buddy_tree_pos buddy_tree_left_child(struct buddy_tree_pos pos);

/* Returns the right child node position. Does not check if that is a valid position */
static force_inline struct buddy_tree_pos buddy_tree_right_child(struct buddy_tree_pos pos);

/* Returns the current sibling node position. Does not check if that is a valid position */
static force_inline struct buddy_tree_pos buddy_tree_sibling(struct buddy_tree_pos pos);

/* Returns the parent node position or an invalid position if there is no parent node */
static force_inline struct buddy_tree_pos buddy_tree_parent(struct buddy_tree_pos pos);
...
// static struct bitset_bucket_range bitset_set_range(unsigned char *bitset, size_t from_pos, size_t to_pos);

// static struct bitset_bucket_range bitset_clear_range(unsigned char *bitset, size_t from_pos, size_t to_pos);
...
static force_inline size_t ceiling_power_of_two(size_t value);
...
static force_inline float approximate_square_root(float f);

/*
 Implementation
*/

static const unsigned int BUDDY_RELATIVE_MODE = 1;
...
static force_inline size_t size_for_depth(struct buddy *buddy, size_t depth);
...
struct buddy *buddy_init_alignment(unsigned char *at, unsigned char *main, size_t memory_size,
        size_t alignment) {
    size_t at_alignment, main_alignment, buddy_size, buddy_tree_order, buddy_bits_size;
    struct buddy *buddy;
    ...
    buddy_tree_init(buddy->buddy_tree, (uint8_t) buddy_tree_order, &buddy_bits_size);
...
void *buddy_malloc(struct buddy *buddy, size_t requested_size) {
    ...
    PRINT_INFO("target depth is %lu, pos depth %lu, idx %lu\n", target_depth, pos.depth, pos.index);
...
/**
 * Split the malloc into two part;
 * First part finds the buddy_tree_posisition
 */
struct buddy_tree_pos buddy_malloc_find_pos(struct buddy *buddy, size_t requested_size) {
    size_t target_depth;
    struct buddy_tree *tree;
    ...
    pos = buddy_tree_find_free(tree, (uint8_t) target_depth);
    if (! buddy_tree_valid(tree, pos)) {
        return INVALID_POS; /* no slot found */
    }
    return pos;
}

/**
 * Second part applies the buddy_tree_posisition
 */
void *buddy_malloc_apply(struct buddy *buddy, struct buddy_tree_pos pos) {
    struct buddy_tree *tree = buddy_tree(buddy);
...
static force_inline struct buddy_tree *buddy_tree(struct buddy *buddy) {
...
static void buddy_toggle_virtual_slots(struct buddy *buddy, unsigned int state) {
    ...
    effective_memory_size = buddy_effective_memory_size(buddy);
    PRINT_INFO("effective memory size: 0x%lx, memory_size: 0x%lx\n", effective_memory_size, memory_size);
    if (effective_memory_size == memory_size) {
        return;
    }
...
static force_inline size_t size_for_order(uint8_t order, uint8_t to);
static force_inline size_t buddy_tree_index_internal(struct buddy_tree_pos pos);
...
static force_inline unsigned char *buddy_tree_bits(struct buddy_tree *t);
static void buddy_tree_populate_size_for_order(struct buddy_tree *t);
static force_inline size_t buddy_tree_size_for_order(struct buddy_tree *t, uint8_t to);
...
static force_inline struct internal_position buddy_tree_internal_position_order(
        size_t tree_order, struct buddy_tree_pos pos) {
...
static force_inline struct internal_position buddy_tree_internal_position_tree(
        struct buddy_tree *t, struct buddy_tree_pos pos) {
    struct internal_position p = {0};
    size_t total_offset, local_index;
...
static struct buddy_tree *buddy_tree_init(unsigned char *at, uint8_t order, size_t *size) {
    ...
    *size = buddy_tree_sizeof(order);
    PRINT_INFO("buddy_tree size: 0x%lx\n", *size);
    memset(at, 0, *size);
    t->order = order;
...
static force_inline size_t buddy_tree_size_for_order(struct buddy_tree *t,
         uint8_t to) {
     return *(t->data+t->size_for_order_offset+to);
}

static void write_to_internal_position(unsigned char *bitset, struct internal_position pos, size_t value) {
    struct bitset_bucket_range clear_rng =  bitset_clear_range(bitset,
        pos.bitset_location,
        pos.bitset_location+pos.local_offset-1);
    size_t to = clear_rng.to;
    if (value) {
        struct bitset_bucket_range set_rng = bitset_set_range(bitset,
            pos.bitset_location,
            pos.bitset_location+value-1);
        if (set_rng.to > clear_rng.to) {
            to = set_rng.to;
        } else {
            to = clear_rng.to;
        }
    }
    shmem_flush_cpu_cache(bitset+clear_rng.from, to-clear_rng.from+1);
}
 ...
 * @param to_pos end position (exclusive)
 */
// static struct bitset_bucket_range bitset_clear_range(unsigned char *bitset, size_t from_pos, size_t to_pos) {
...
#error Unsupported platform
    ...
    for (i = 0; i < (sizeof all_set / sizeof *all_set); i++) {
        if (value >= all_set[i]) {
            value >>= count[i];
            result += count[i];
        }
